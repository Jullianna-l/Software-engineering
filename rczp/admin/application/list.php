...
?>
<HTML ><HEAD id=Head1><TITLE>应聘信息列表</TITLE>

    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, job-scalable=no" />
...
<FORM name="form" method="post" action="" >
    ...
    <div class="panel admin-panel">
        ...
        <div class="panel-head"><strong class="icon-reorder"> 应聘列表</strong> </div>
        ...
        <table class="table table-hover text-center">
            ...
            $sql="select a.*,j.title from application a,job j where j.id=a.job_id ";//拼接留言sql语句

            $keyword=$_GET["keyword"];
            $query1="select count(a.id) as total from application a,job j where j.id=a.job_id  ";

            if(!empty($keyword)){
                $sql=$sql." and (   j.title like '%".$keyword."%' or  j.treatment like '%".$keyword."%' or j.requirement like '%".$keyword."%')  ";
                $query1=$query1." and (   j.title like '%".$keyword."%' or  j.treatment like '%".$keyword."%' or j.requirement like '%".$keyword."%')  ";
            ...
            while($row=mysqli_fetch_array($result)){
                ...
                $sqlf2="select *  from user where id='".$row['user_id']."'";
                $rs2=mysqli_query($con,$sqlf2);
                $user=mysqli_fetch_array($rs2);

            ?>
                    <tr>
                        <td  align="center" height="28"  ><?=$company['name']?></td>

                        <td  align="center" height="28"  ><?=$row['title']?></td>
